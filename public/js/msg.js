...
async function emojiList() {
  ...
  if (isMobile) {
    ...
    const prevMenu2 = document.getElementById("userPopup");
    if (prevMenu2) prevMenu2.remove();
    const prevMenu3 = document.getElementById("emojiChooser");
    if (prevMenu3) prevMenu3.remove();
  }
  return new Promise(async (resolve) => {
    if (emojiOpened == false) {
      ...
      list.classList.add("emojiChooser");
    ...
    json = {
      ...
      data: JSON.stringify({
        ...
        author: {
          ...
          pfp: s.account.pfp,
          pfpDecor: s.account.decor,
          theme: s.general.appTheme,
          abtMe: s.account.abtMe,
        },
...
async function messageReceiver(event) {
  ...
  if (message.type == "messageCreate") {
    ...
    content.innerHTML = `<p class='lt'><i>[DELETED]</i></p>`;
...
async function buildMessage(message, type) {
  if (type == "message") {
    ...
    const pfpDecor = messageData.author.pfpDecor;
    const dontShowPfp = username == msgId || !s.general.showPfps;
    const showDecor = s.general.showPfpDecor;
    ...
    pfpDiv.style.setProperty(
      "--img",
      `url(${pfp || "/media/image/icons/profile.png"})`
    );
    if (showDecor) {
      pfpDiv.style.setProperty(
        "--pfp-decor",
        `url(/media/image/decors/${pfpDecor?.toLowerCase() || ""}.svg)`
      );
    }
    ...
    topDiv.innerHTML += "<br>";
    topDiv.onclick = (e) => {
      popoutUser(JSON.parse(message.data).author, e);
    };
...
function replyMessage(id, auth) {
  ...
  document.getElementById("bottom-nav").appendChild(inputType);
  setTimeout(() => {
...
function editMessage(id) {
  ...
  const prevType = document.getElementById("inputType");
  if (prevType) prevType.remove();
  console.log(document.getElementById(`unparsed-${id}`).value);
...
function popoutUser(json, e) {
  ...
  const decor = json.pfpDecor;
  const pfp = json.pfp;
  const bio = json.abtMe;
  const theme = json.theme;
  const color = json.nameColor;
  const username = json.username;
  const popup = document.createElement("div");
  popup.id = "userPopup";
  popup.setAttribute("theme", theme);
  popup.classList.add("user-popup");
  if (isMobile) popup.classList.add("mobile");
  const nameDiv = document.createElement("div");
  nameDiv.style.color = color;
  nameDiv.innerHTML = clean(name);
  nameDiv.classList.add("name");
  const bioDiv = document.createElement("div");
  bioDiv.innerHTML = clean(bio).replace(/\n/g, "<br />");
  bioDiv.classList.add("bio");
  const pfpDiv = document.createElement("div");
  pfpDiv.style = `--img: url(${pfp}); --pfp-decor: url(/media/image/decors/${decor.toLowerCase()}.svg)`;
  pfpDiv.classList.add("pfp");
