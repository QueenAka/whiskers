...
function loadDecors() {
  const files = fs.readdirSync(
    path.join(__dirname, "public/media/image/decors")
  );
  decorList = files.map((file) => {
    const name = file.split(".")[0];
    return {
      name: name.charAt(0).toUpperCase() + name.slice(1).toLowerCase(),
      url: `/media/image/decors/${file}`,
      fileType: "image/" + file.split(".")[1],
    };
  });
}
...
async function tenorSearch(q, next) {
  return new Promise((resolve) => {
    fetch(
      `https://tenor.googleapis.com/v2/search?q=${q}&key=${TENOR}&limit=20&pos=${
        next || 0
      }`
    )
      .then((res) => res.json())
...
app.get("/api/decors", async (req, res) => {
  loadDecors();
  res.json(decorList);
});
